...
# NOTE(yuming): SimpleHTMLDocument is a simplified version of HTMLDocument
class SimpleHTMLDocument:
    def __init__(self, elements: List[Element]):
        self.elements = elements
        self.pages = [1] if elements else []

    @classmethod
    def from_elements(cls, elements: List[Element]):
        return cls(elements)

    def after_element(self, el: Element):
        try:
            idx = self.elements.index(el)
            return SimpleHTMLDocument(self.elements[idx + 1:])
        except ValueError:
            return SimpleHTMLDocument([])

    def before_element(self, el: Element):
        try:
            idx = self.elements.index(el)
            return SimpleHTMLDocument(self.elements[:idx])
        except ValueError:
            return SimpleHTMLDocument([])
...
class SECDocument(SimpleHTMLDocument):
    filing_type = None
    ...
    def get_table_of_contents(self) -> SimpleHTMLDocument:
    ...
    def _get_toc_sections(
        self, section: SECSection, toc: SimpleHTMLDocument
    ) -> Tuple[Text, Text]:
...
def get_narrative_texts(
    doc: SimpleHTMLDocument, up_to_next_title: Optional[bool] = False
) -> List[Text]:
